(function() {
  ...
  var hostkey = 'http://showypro.com'.replace('http://', '').replace('https://', '');
  ...
  window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
        ...
        net.silent('http://showypro.com'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
          check(true);
        }, function() {
          check(false);
        }, false, { dataType: 'text' });
  ...
  window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
    window.rch_nws[hostkey].typeInvoke('http://showypro.com', function() {
      ...
      client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
        ...
        function sendResult(uri, html) {
          $.ajax({
            url: 'http://showypro.com/rch/' + uri + '?id=' + rchId,
            type: 'POST',
            data: html,
            async: true,
            cache: false,
        ...
        function result(html) {
          ...
          if (typeof CompressionStream !== 'undefined' && html && html.length > 1000) {
            ...
            new Response(compressedStream).arrayBuffer()
              ...
              .then(null, function() { sendResult('result', html); });
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://showypro.com/js/nws-client-es5.js?v21042026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
  ...
  // ==========================================================
  // TEMPLATES AND CSS REGISTRATION
  // ==========================================================
  ...
  Lampa.Template.add('lampac_css_v2', 
    "<style>" +
    "@charset 'UTF-8';" +
    ".online-prestige{position:relative;-webkit-border-radius:.3em;border-radius:.3em;background-color:rgba(0,0,0,0.4);display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;transition:transform 0.15s ease, background-color 0.15s ease;}" +
    ...
    ".online-prestige__img{position:relative;width:13em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;min-height:8.2em;overflow:hidden;border-radius:.3em 0 0 .3em;}" +
    ...
    ".online-prestige__viewed{position:absolute;top:0.8em;left:0.8em;background:rgba(0,0,0,0.65);-webkit-border-radius:100%;border-radius:100%;padding:.25em;font-size:.76em;z-index:2;}" +
    ".online-prestige__viewed>svg{width:1.5em !important;height:1.5em !important;fill:#00ffcc;}" +
    ".online-prestige__episode-number{position:absolute;top:0;left:0;right:0;bottom:0;display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-webkit-align-items:center;-moz-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-webkit-justify-content:center;-moz-box-pack:center;-ms-flex-pack:center;justify-content:center;font-size:2em;font-weight:700;text-shadow:0 2px 8px rgba(0,0,0,0.8);}" +
    ...
    ".online-prestige__timeline>.time-line{display:block !important;height:4px;border-radius:2px;background:rgba(255,255,255,0.15);}" +
    ".online-prestige__title{font-size:1.7em;font-weight:600;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:1;line-clamp:1;-webkit-box-orient:vertical}" +
    "@media screen and (max-width:480px){.online-prestige__title{font-size:1.4em}}" +
    ".online-prestige__time{padding-left:2em;color:#aaaaaa;font-size:0.9em;}" +
    ".online-prestige__info{display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-webkit-align-items:center;-moz-box-align:center;-ms-flex-align:center;align-items:center;color:#bbbbbb;font-size:0.95em;}" +
    ...
    ".online-prestige__quality{padding-left:1em;white-space:nowrap;color:#ff4444;font-weight:600;}" +
    ".online-prestige .online-prestige-split{font-size:.8em;margin:0 0.8em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;color:#666666;}" +
    ".online-prestige.focus{transform:scale(1.02);background-color:rgba(255,255,255,0.15);box-shadow:0 8px 24px rgba(0,0,0,0.6);z-index:10;}" +
    ".online-prestige.focus::after{content:'';position:absolute;top:-0.3em;left:-0.3em;right:-0.3em;bottom:-0.3em;-webkit-border-radius:.5em;border-radius:.5em;border:solid .25em #ffffff;pointer-events:none;}" +
    ".online-prestige+.online-prestige{margin-top:1.2em}" +
    ".online-prestige-watched{padding:1em;background:rgba(255,255,255,0.05);border-radius:0.4em;}" +
    ".online-prestige-watched__icon>svg{width:1.5em;height:1.5em;color:#ff2a2a;}" +
    ".online-prestige-watched__body{padding-left:1em;padding-top:.1em;display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;color:#e0e0e0;font-size:0.95em;}" +
    ".online-prestige-watched__body>span+span::before{content:' ● ';vertical-align:top;display:inline-block;margin:0 .5em;color:#777777;}" +
    ...
    ".online-prestige-rate>svg{width:1.3em !important;height:1.3em !important;fill:#f5c518;}" +
    ".online-prestige-rate>span{font-weight:700;font-size:1.1em;padding-left:.5em;color:#ffffff;}" +
    ".online-empty{line-height:1.4;padding:2em;text-align:center;}" +
    ".online-empty__title{font-size:1.8em;font-weight:700;margin-bottom:.3em;color:#ffffff;}" +
    ".online-empty__time{font-size:1.2em;font-weight:300;margin-bottom:1.6em;color:#aaaaaa;}" +
    ".online-empty__buttons{display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;justify-content:center;}" +
    ".online-empty__buttons>*+*{margin-left:1em}" +
    ".online-empty__button{background:rgba(255,255,255,0.1);font-size:1.2em;padding:.6em 1.5em;-webkit-border-radius:.4em;border-radius:.4em;margin-bottom:2.4em;cursor:pointer;transition:all 0.15s ease;}" +
    ".online-empty__button.focus{background:#ffffff;color:#000000;transform:scale(1.05);font-weight:700;}" +
    "</style>"
  );
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account('http://showypro.com/lite/withsearch'), function(json) {
    ...
    this.getLastChoiceBalanser = function() {
      ...
      else return Lampa.Storage.get('online_balanser', filter_sources.length ? filter_sources[0] : '');
    };
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        ...
        if (filter_sources.length) {
          ...
          else balanser = Lampa.Storage.get('online_balanser', filter_sources[0]);
    ...
    this.toPlayElement = function(file) {
      return {
        title: file.title, url: file.url, quality: file.qualitys, timeline: file.timeline,
        subtitles: file.subtitles, segments: file.segments, callback: file.mark,
        season: file.season, episode: file.episode, voice_name: file.voice_name, thumbnail: file.thumbnail
      };
    };
    ...
    this.orUrlReserve = function(data) {
      if (data.url && typeof data.url == 'string' && data.url.indexOf(" or ") !== -1) {
        ...
        data.url = urls[0]; data.url_reserve = urls[1];
      }
    };
          ...
          _this5.getFileUrl(item, function(json, json_call) {
            if (json && json.url) {
              ...
              if (json.vast && json.vast.url) {
                first.vast_url = json.vast.url; first.vast_msg = json.vast.msg;
                first.vast_region = json.vast.region; first.vast_platform = json.vast.platform;
                first.vast_screen = json.vast.screen;
              }
              _this5.orUrlReserve(first); _this5.setDefaultQuality(first);
                  ...
                  if (elem == item) cell.url = json.url;
                    ...
                    if (elem.method == 'call') {
                      if (Lampa.Storage.field('player') !== 'inner') {
                        cell.url = elem.stream; delete cell.quality;
                      } else {
                        cell.url = function(call) {
                          _this5.getFileUrl(elem, function(stream, stream_json) {
                            if (stream.url) {
                              ...
                              _this5.orUrlReserve(cell); _this5.setDefaultQuality(cell);
                              elem.mark();
                            } else {
                              cell.url = ''; Lampa.Noty.show(Lampa.Lang.translate('lampac_nolink'));
                            }
                            call();
        ...
        if (items.length == 1 && items[0].method == 'link' && !items[0].similar) {
          ...
          if (videos.length) {
            if (buttons.length) {
              ...
              var find_voice_name = buttons.find(function(v) { return v.text == select_voice_name; });
              var find_voice_active = buttons.find(function(v) { return v.active; });
    ...
    this.similars = function(json) {
      ...
      json.forEach(function(elem) {
        ...
        item.on('hover:enter', function() {
          _this6.reset(); _this6.request(elem.url);
        }).on('hover:focus', function(e) {
          last = e.target; scroll.update($(e.target), true);
        });
        scroll.append(item);
      });
      this.filter({
    ...
    this.clearImages = function() {
      images.forEach(function(img) {
        img.onerror = function() {}; img.onload = function() {}; img.src = '';
      });
      images = [];
    };
    ...
    this.getEpisodes = function(season, call) {
      ...
      if (typeof tmdb_id == 'number' && object.movie.name) {
        Lampa.Api.sources.tmdb.get('tv/' + tmdb_id + '/season/' + season, {}, function(data){
          episodes = data.episodes || []; call(episodes);
    ...
    this.draw = function(items) {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          if (element.quality) {
            element.qualitys = element.quality; element.quality = Lampa.Arrays.getKeys(element.quality)[0];
          }
          Lampa.Arrays.extend(element, {
            voice_name: voice_name,
            info: voice_name.length > 60 ? voice_name.substr(0, 60) + '...' : voice_name,
            quality: '', time: Lampa.Utils.secondsToTime((episode ? episode.runtime : object.movie.runtime) * 60, true)
          });
          ...
          var info = [];
          if (element.season) {
            element.translate_episode_end = _this8.getLastEpisode(items); element.translate_voice = element.voice_name;
          }
          ...
          if (serial && !episode) {
            ...
            img.onload = function() {
              image.addClass('online-prestige__img--loaded'); loader.remove();
            ...
            images.push(img); element.thumbnail = img.src;
          }
          ...
          element.mark = function() {
            ...
            _this8.watched({
              balanser: balanser, balanser_name: Lampa.Utils.capitalizeFirstLetter(sources[balanser] ? sources[balanser].name.split(' ')[0] : balanser),
              voice_id: choice.voice_id, voice_name: voice_name_text, episode: element.episode, season: element.season
            });
          };
          element.unmark = function() {
            ...
            if (viewed.indexOf(hash_behold) !== -1) {
              ...
              Lampa.Storage.remove('online_view', hash_behold); html.find('.online-prestige__viewed').remove();
            }
          };
          ...
          html.on('hover:enter', function() {
            ...
            last = e.target; if (params.onFocus) params.onFocus(element, html, data);
    ...
    this.contextMenu = function(params) {
      params.html.on('hover:long', function() {
        function show(extra) {
          ...
          menu.push({ title: Lampa.Lang.translate('lampac_clear_all_timecodes'), clearalltime: true });
          if (extra.quality) menu.push({ title: Lampa.Lang.translate('settings_server_links'), copylink: true });
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              ...
              if (a.clearalltime) params.onClearAllTime();
    ...
    this.doesNotAnswer = function(er) {
      ...
      html.find('.change').on('hover:enter', function() {
        clearInterval(balanser_timer); filter.render().find('.filter--sort').trigger('hover:enter');
      });
      scroll.clear(); scroll.append(html); this.loading(false);
      balanser_timer = setInterval(function() {
        ...
        if (tic == 0) {
          ...
          var next = keys[indx + 1]; if (!next) next = keys[0];
          balanser = next;
    ...
    this.destroy = function() {
      network.clear(); this.clearImages(); files.destroy(); scroll.destroy();
      clearInterval(balanser_timer); clearTimeout(life_wait_timer);
    };
  }

  // ==========================================================
  // PLUGIN REGISTRATION & BUTTON INJECTION
  // ==========================================================
  function startPlugin() {
    ...
    function openShowyPro(object) {
      ...
      Lampa.Activity.push({
        url: '', title: 'Showy RU v2', component: 'showy_ru_lampac_v2',
    ...
    if (Object.prototype.toString.call(plugins) != '[object Array]') plugins = [plugins];
  ...
  if (!window.showy_ru_lampac_v2_plugin) startPlugin();
})();
